	...
	case "Save" :   /* Only happens on a new record */
		if (validate($_POST)) {
			...
			$sqlStmt = 	sprintf($sql, 
				...
				mysqli_real_escape_string($vbsDBi, (WAITLIST_ONLY ? 'W' : 'Y')),
	...
	case "Update" :
		if (validate($_POST)){
			...
			if ($_POST['submit']=="Register") {
				...
				if (WAITLIST_ONLY){
					$sqlStmt .= ", registered='W', confo='" . $_SESSION['confoNo'] . "' ";
				} else {
...
<div id="Student" class="gridContainer">
	...
	<table>
		...
		<tr><td class="label">*&nbsp;Birthdate</td><td class="value"><input name="birthdate" type="date" value="<?php echo $row_rsStudent['birthdate']; ?>" min="2003-07-11" max="2013-07-11"></td></tr>
...
?>
        ...
        <?php } else { if ($waitlisted) { ?>
        	<input type="submit" name="submit" value="Update">&nbsp;&nbsp;<input type="submit" name="submit" value="Un-waitlist">
